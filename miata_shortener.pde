...
void draw() {
  if (cycles < img.width/3) {
    ...
    background(0);
     cycles++;
     image(newImg, 0, 0, newImg.width/2, newImg.height/2); 
 ...
 for (int i = 0; i<columns-2; i++) {
   ...
   for (int i = columnToRemove + 1; i < ret.width; i++) {
     for (int j = 0; j< ret.height; j++) {
       if (shiftDirection == -1){
         int newJ = j+1;
         ret.pixels[(i - 1) + (newJ-1)*ret.width] = original.pixels[i + ((newJ)*original.width)]; 
       } else {
         ret.pixels[(i - 1) + j*ret.width] = original.pixels[i + ((j*shiftDirection)*original.width)]; 
       }
